...
import { TweenLite } from 'gsap/all.js'
import { Power1 } from 'gsap/all.js'
import { BoxHelper, Scene } from 'three'
...
class Camera {
    ...
    setDiffCam() {
        ...
        this.instance = new THREE.PerspectiveCamera(65, window.innerWidth / window.innerHeight, 1, 2000)
    ...
    testingCam() {
        ...
        this.instance = new THREE.PerspectiveCamera(65, window.innerWidth / window.innerHeight, 1, 2000)
        this.instance.position.set(10, 25, 15)

        new OrbitControls(this.instance, this.renderer.domElement)
    }
    ...
    setAngle() {
        ...
        // Items
        this.angle.items = {
            default: new THREE.Vector3(0, 15, -25),
    ...
    setInstance() {
        // Set up
        this.instance = new THREE.PerspectiveCamera(65, window.innerWidth / window.innerHeight, 1, 100)
    ...
    setOrbitControls() {
        ...
        eventHandler.on('tick', () => {
            if (world && ufo) {
                camera.target.x = ufo.position.x
                camera.target.z = ufo.position.z
                camera.target.y = ufo.position.y
            }
        })
        // Debug
        // if (this.debug) {
